...
parser.add_argument('--train_label_ratio', type=float, default=0.25, help='ratio of labeled frames used for training in trainlist')
...
parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints/encoder_estimater/semi', help='models are saved here')
    ...
    for epoch in range(500):
        ...
        if (epoch + current_es_index + 1) % 1 == 0:
            ...
            model.save_network(model.estimater, 'estimater', '%d_%f_%.8f' % (epoch + current_es_index + 1, test_wld_err, model.get_es_lr()),
                                   opt.gpu_id)
        # learning rate decay
        if (epoch + current_es_index + 1) % 10 == 0 and epoch >= 0:
