...
parser.add_argument('--es_version', type=str, default='v6',
...
parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints/all/full_v6', help='models are saved here')
...
parser.add_argument('--pretrain_encoder', type=str, default='checkpoints/encoder_estimater/full_v6/59_9.727027_0.00000031_net_encoder.pth',
                         help='pre-trained encoder dict path: None or str')
parser.add_argument('--pretrain_decoder', type=str, default='checkpoints/autoencoder/full/30_0.006366_0.000031_net_decoder.pth',
                         help='pre-trained decoder dict path: None or str')
parser.add_argument('--pretrain_estimater', type=str, default='checkpoints/encoder_estimater/full_v6/0_9.727027_0.00000031_net_estimater.pth',
    ...
    for epoch in range(60):
        ...
        for i, data in enumerate(tqdm(trainloader,0)):
            ...
            estimation_joins = estimation_joins.view(estimation_joins.size(0), 1,
                                                         estimation_joins.size(1))  # [8,1,42]
            '''
            #PCA_mean = PCA_mean.view(PCA_mean.size(0),PCA_mean.size(2))
            #outputs_xyz = torch.bmm(estimation_joins, PCA_coeff.cuda()).cpu()
            outputs_xyz = torch.bmm(estimation_joins, PCA_coeff.cuda()).cpu()
            outputs_xyz = torch.add(PCA_mean,outputs_xyz)
            #outputs_xyz1 = torch.baddbmm(PCA_mean, estimation_joins.cpu(), PCA_coeff)
        ...
        train_loss_all = sum(train_loss_all_batch) / epoch_iter
