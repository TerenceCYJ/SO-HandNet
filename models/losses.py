...
class LossEstimation(nn.Module):
    ...
    def forward(self, input, target):
        ...
        :return:
        '''
        return self.mse_loss(input, target)#refine this function after define the dim of input and target

class LossEstimation_msra_sf(nn.Module):
    def __init__(self, size_average=True):
        super(LossEstimation_msra_sf, self).__init__()
        self.mse_loss=nn.MSELoss(size_average)
    ...
    def forward(self, input, target):
        ...
        :return:
        '''
        eu_loss=self.mse_loss(input, target)#loss of coordinates
        input0=input.view(-1,3,21)
        input0=input0.cpu().detach().numpy()
        target0=target.view(-1,3,21)
        target0 = target0.cpu().detach().numpy()
        list_co = [[0, 1], [1, 2], [2, 3], [3, 4], [0, 5], [5, 6], [6, 7], [7, 8], [0, 9], [9, 10], [10, 11],
                         [11, 12], [0, 13], [13, 14], [14, 15], [15, 16], [0, 17], [17, 18], [18, 19], [19, 20]]
        dis_loss=0
        ori_loss=0

        for i in range(np.size(input0,0)):
            input00=input0[i,:]#[3,21]numpy
            target00=target0[i,:]
            for list_c in list_co:
                #distance
                dis_es=np.linalg.norm((input00[:,list_c[0]]-input00[:,list_c[1]]),ord=2)
                dis_gt=np.linalg.norm((target00[:,list_c[0]]-target00[:,list_c[1]]),ord=2)
                #dis_es=np.sqrt((input00[:,list_c[0]])*((input00[:,list_c[1]]).T))
                #dis_gt = np.sqrt((target00[:, list_c[0]]) * ((target00[:, list_c[1]]).T))
                if dis_gt != 0:
                    dis_ratio=abs(dis_es-dis_gt)/dis_gt
                else:
                    dis_ratio=0
                dis_loss+=dis_ratio
                '''
                #ori
                v_es=input00[:,list_c[0]]-input00[:,list_c[1]]
                v_es=v_es/np.linalg.norm(v_es,ord=2)
                v_gt=target00[:, list_c[0]]-target00[:, list_c[1]]
                if np.linalg.norm(v_gt, ord=2)!= 0:
                    v_gt = v_gt / np.linalg.norm(v_gt, ord=2)
                else:
                    v_gt = v_es
                ori_ratio=(v_gt-v_es)/v_gt
                ori_ratio=np.linalg.norm(ori_ratio, ord=1)#L1 NORM
                ori_loss+=ori_ratio
                '''
        dis_loss=dis_loss/(np.size(input0)*len(list_co))#mean
        #ori_loss = ori_loss / (np.size(input0) * len(list_co))  # mean
        #soft_loss = dis_loss+ori_loss
        #all_loss=eu_loss+dis_loss+0.1*ori_loss
        all_loss = eu_loss + dis_loss
        return all_loss#refine this function after define the dim of input and target

class LossEstimation_nyu_sf(nn.Module):
    def __init__(self, size_average=True):
        super(LossEstimation_nyu_sf, self).__init__()
        self.mse_loss=nn.MSELoss(size_average)
    ...
    def forward(self, input, target):
        ...
        '''
        eu_loss=self.mse_loss(input, target)#loss of coordinates
        input0=input.view(-1,3,14)
        input0=input0.cpu().detach().numpy()
        target0=target.view(-1,3,14)
        target0 = target0.cpu().detach().numpy()
        list_co = [[0, 1], [1, 13], [11, 13], [3, 13], [2, 3], [4, 5], [5, 13], [6, 7], [7, 13], [8, 9], [9, 10],
                         [10, 12], [12, 13]]
        dis_loss=0
        ori_loss=0
...
class LossEstimation_icvl_sf(nn.Module):
    def __init__(self, size_average=True):
        super(LossEstimation_icvl_sf, self).__init__()
        self.mse_loss=nn.MSELoss(size_average)
    ...
    def forward(self, input, target):
        ...
        '''
        eu_loss=self.mse_loss(input, target)#loss of coordinates
        input0=input.view(-1,3,16)
        input0=input0.cpu().detach().numpy()
        target0=target.view(-1,3,16)
        target0 = target0.cpu().detach().numpy()
        list_co = [[0, 1], [1, 2], [2, 3], [0, 4], [4, 5], [5, 6], [0, 7], [7, 8], [8, 9],
